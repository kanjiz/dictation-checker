...
### 新規ファイル（src/download.ts）

ダウンロードロジックを3つの関数に分離する。
...
#### `handleDownloadKeydown`（公開インターフェース）
...
`shortcuts.ts` の `ShortcutConfig` を受け取り、内部で `shortcuts.download` を使ってキーを照合する。`handlePlayerKeydown` と引数の順序・型を揃える。

`shortcuts.download` に一致するキーを検出したとき：

1. `event.preventDefault()` でブラウザのページ保存を抑止する
2. `generateFilename(new Date())` でファイル名を生成する
3. `downloadText(editor.value, filename)` でファイルをダウンロードさせる
4. `announce('保存しました')` でスクリーンリーダーに通知する
5. `downloadStatus` の `hidden` を外し、テキストを「保存しました」にセットする
6. 2秒後に `downloadStatus` に `hidden` を付与してテキストをクリアする

Ctrl+S 連打時、`announce()` は保存ごとに呼ばれる。`announce()` 内部には `textContent === message` のガードがあるため、累積タイマーがあっても最後の呼び出しから2秒後に一度だけ `statusRegion` がクリアされる（既存の動作に委ねる）。`downloadStatus` の非表示タイマーは `clearTimeout` で前回分をキャンセルしてから新たにセットし、「最後の保存から2秒間」視覚表示が維持されるようにする。`downloadStatus` と `#status-region` の非表示タイミングは独立しており、これは意図した設計である。

`shortcuts.download` に一致しないキーは何もしない。

### shortcuts.ts の変更

`ShortcutConfig` に `download` を追加し、`DEFAULT_SHORTCUTS` にデフォルト値を追加する。
...
`editor` の `keydown` ハンドラに `handleDownloadKeydown` の呼び出しを追加する。
...
`downloadStatus` は `document.getElementById('download-status')` で取得する。

### CSS（src/style.css）

ダウンロードステータスのスタイルを追加する。
...
### テスト（src/download.test.ts）

`generateFilename`・`downloadText`・`handleDownloadKeydown` の単体テストを vitest で追加する。
...
| 通常テキストのダウンロード | `URL.createObjectURL` が呼ばれ、`<a>` の `click` が呼ばれる |
| 空テキストのダウンロード | 空でも `downloadText` が正常に動作する |
| Ctrl+S でダウンロードされる | `downloadText` が呼ばれ、`announce` が呼ばれ、`download-status` が表示される |
| 2秒後にステータスが非表示になる | `hidden` が付与される |
| Ctrl+S 連打時 | `announce` が保存ごとに呼ばれ、`downloadStatus` の非表示タイマーはリセットされ最後の保存から2秒後に `hidden` が付与される |
| 他のキーは無視 | `shortcuts.download` に一致しないキーで `downloadText` が呼ばれない |
...
| `src/style.css` | 変更（ダウンロードステータススタイル追加） |
